...
class ReportsController extends Controller {
    ...
    public function get_outstanding(Request $request) {
        ...
        if ($request->fdate != "" && $request->tdate != "") {
            $query->whereRaw("tbl_invoice.created_at > '$request->fdate' and tbl_invoice.created_at < '$request->tdate'");
        }
    ...
    public function get_outstanding_invoice_wise(Request $request) {
        ...
        if ($request->fdate != "" && $request->tdate != "") {
            $request->tdate = date('Y-m-d H:i:s', strtotime($request->tdate . " 23:59:59"));
            $query->whereRaw("tbl_invoice.created_at > '$request->fdate' and tbl_invoice.created_at < '$request->tdate'");
        }
        $query = $query->whereNotIn('tbl_invoice.is_pay', [1, 3])
                        ->whereNotIn('tbl_invoice.is_invoice', [2, 3])->orderBy('tbl_invoice.order_id')->get();
                    ...
                    if ($count_i == $i) {
                        ...
                        for ($y = 0; $y < sizeof($data[$index]['invoice']); $y++) {
                            ...
                            if ($date == $data[$index]['invoice'][$y]['date']) {
                                ...
                                // sizeof($data[$index]['invoice']) - 1 or sizeof($data[$index]['invoice']) == 1
    ...
    public function get_collection(Request $request) {
        ...
        if (sizeof($query)) {
            ...
            return ['status' => true, 'data' => $data_sort, 'collection_total' => $collection_total];
        } else {
            return ['status' => false];
        }
    }
    ...
    public function get_activate(Request $request) {
        ...
        if ($request->fdate != "" && $request->tdate != "") {
            $query->whereRaw("tbl_main_warehouse_receive_item.activated_date > '$request->fdate' and tbl_main_warehouse_receive_item.activated_date < '$request->tdate'");
        } else {
    ...
    public function sale_return_imeis_details(Request $request){
        if ( $request->input('client') ) {
            return DB::table('tbl_warehouse_issue_return')
                ...
                ->join('tbl_region', 'tbl_city.id', '=', 'tbl_region.city_id')->get();
        }
        $columns = [];
        ...
        $query = DB::table('tbl_warehouse_issue_return')
                ...
                ->join('tbl_city', 'tbl_city.id', '=', 'tbl_retailer_outlet.city_id')
                ->join('tbl_state', 'tbl_state.id', '=', 'tbl_city.state_id')
                ->join('tbl_region', 'tbl_city.id', '=', 'tbl_region.city_id')->orderBy($columns[$column], $dir);
