...
class ReportsController extends Controller {
    ...
    public function get_outstanding_ledger(Request $request) {
        $data = [];
        $total = 0;
        $query = DB::table('tbl_retailer_outlet')
                ->select(
                       DB::raw("(SELECT ROUND(SUM(Credit)-sum(COALESCE(Collection,0))) FROM `tbl_ledger` WHERE retailer_id=tbl_retailer_outlet.id and is_reverse=0) as outstanding")
                        , 'tbl_retailer_outlet.name as outletname', 'tbl_retailer_outlet.credit_limit'
                        , 'tbl_retailer_outlet.address'
                        , 'tbl_retailer.retailer_no', 'tbl_city.name as cityname', 'tbl_region.name as region_name'
                )
        ...
        $query = $query->orderBy('tbl_retailer_outlet.city_id')->get();
        if (sizeof($query)) {
            for ($i = 0; $i < sizeof($query); $i++) {
                //if ($query[$i]->outstanding != 0) {
                //  $data[$i] = $query[$i];
                $total += $query[$i]->outstanding;
                // }
            }
            return ['status' => true, 'data' => $query, 'total' => $total];
        } else {
            return ['status' => false];
        }
    }
...
//        if (($request->fdate == "" && $request->tdate == "") || ($request->month == "")) {
//            return ['status' => false];
//        }
        $data = [];
        $data_sort = [];
        ...
        if ($request->date_type == "monthly") {
            ...
            $to = date('Y-m-t H:i:s', strtotime($request->month . " 23:59:59"));
            $query->whereRaw("tbl_invoice.created_at > '$from' and tbl_invoice.created_at < '$to'");
        }
...
//        }
    ...
    public function get_collection(Request $request) {
        ...
        // $query->where(['tbl_retailer_collection.is_reversible' => 0]);
        if ($request->state != "all") {
            $query->where(['tbl_city.state_id' => $request->state]);
        }
        if ($request->city != "all" && $request->city != null) {
        ...
        if (sizeof($query)) {
            ...
            $day_total_array = $this->create_month_zero();
            for ($j = 1; $j <= sizeof($data); $j++) {
                $data_sort[$data[$j]['outlet_id']]['collection'] = $this->create_month();
                $sizeof_collection_data = sizeof($data[$j]['collection_data']) - 1;
                for ($z = 0; $z < sizeof($data[$j]['collection_data']); $z++) {
                    ...
                    if ($data[$j]['collection_data'][$z]['date'] == $date) {
                        ...
                        if ($sizeof_collection_data == $z) {
                            $day_total_array[$date] = $day_total_array[$date] + $day_total;
                            $day_total = 0;
                        }
                    } else {
                        $z--;
    ...
    public function activation_sort_total($data) {
        ...
        for ($i = 0; $i < sizeof($all_products); $i++) {
            $products_total[$all_products[$i]->name] = 0;
        }
        for ($i = 0; $i < sizeof($data); $i++) {
            for ($j = 1; $j <= sizeof($data[$i]['pro']); $j++) {
                ...
                $products_total[$data[$i]['pro'][$j][1]['p_name']] = $products_total[$data[$i]['pro'][$j][1]['p_name']] += sizeof($data[$i]['pro'][$j]);
            }
        }
        return ['status' => true, 'data' => $result, 'total' => $total, 'products_total' => $products_total];
    }
    ...
    public function get_sale_return(Request $request) {
        ...
        if ($request->warehouse_id != "") {
            $query->where(['tbl_retailer_order.warehouse_id' => $request->warehouse_id]);
        }
        $query = $query->get();
        if (sizeof($query)) {
            for ($i = 0; $i < sizeof($query); $i++) {
    ...
    public function sale_return_products_sort($data) {
        ...
        for ($i = 0; $i < sizeof($data); $i++) {
            ...
            $sort[$index]['outlet_id'] = $data[$i]['outlet']['id'];
    ...
    public function sale_return_color_sort($data) {
        ...
        $total = 0;
        for ($i = 0; $i < sizeof($data); $i++) {
            $sort[$index]['outlet_id'] = $data[$i]['outlet_id'];
    ...
    public function sale_return_details(Request $request) {
        ...
        $query = DB::table('tbl_warehouse_issue_return')
                ->join('tbl_imei', 'tbl_warehouse_issue_return.item_id', '=', 'tbl_imei.item_id')
                ->join('tbl_item', 'tbl_item.id', '=', 'tbl_warehouse_issue_return.item_id')
                ->join('tbl_product_color', 'tbl_product_color.id', '=', 'tbl_item.product_color_id')
                ->join('tbl_product', 'tbl_product.id', '=', 'tbl_product_color.product_id')
                ->join('tbl_invoice', 'tbl_invoice.id', '=', 'tbl_warehouse_issue_return.invoice_id')
                ->join('tbl_retailer_order', 'tbl_retailer_order.id', '=', 'tbl_invoice.order_id')
                ->join('tbl_retailer_outlet', 'tbl_retailer_outlet.id', '=', 'tbl_retailer_order.outlet_id')
                ->where('tbl_retailer_outlet.id', '=', $request->outlet_id);
        if ($request->warehouse_id != '') {
            $query = $query->where('tbl_retailer_order.warehouse_id', '=', $request->warehouse_id);
        }
        if ($request->fdate != "" && $request->tdate != "") {
            $from = date('Y-m-d H:i:s', strtotime($request->fdate . " 00:00:00"));
            $to = date('Y-m-d H:i:s', strtotime($request->tdate . " 23:59:59"));
            $query = $query->whereRaw("tbl_warehouse_issue_return.created_at > '$from' and tbl_warehouse_issue_return.created_at < '$to'");
        }
        $query = $query->select('tbl_invoice.id as invoice_id', 'tbl_invoice.order_id', 'tbl_imei.imei1'
                        , 'tbl_imei.imei2', 'tbl_product_color.color', 'tbl_product.name as p_name'
                        , 'tbl_retailer_outlet.name as outlet_name', 'tbl_retailer_outlet.phone_no', 'tbl_retailer_outlet.address'
                        , 'tbl_warehouse_issue_return.created_at as return_time')
                ->orderBy('tbl_warehouse_issue_return.created_at', 'desc')->orderBy('tbl_product.id', 'asc')
                ->orderBy('tbl_product_color.id', 'asc')
                ->get();
        if (sizeof($query)) {
            return ['status' => true, 'data' => $query, 'total' => sizeof($query)];
        } else {
    ...
    public function sale_return_imeis_details(Request $request) { 
        $query = DB::table('tbl_warehouse_issue_return')
                ->join('tbl_imei', 'tbl_warehouse_issue_return.item_id', '=', 'tbl_imei.item_id')
                ->join('tbl_item', 'tbl_item.id', '=', 'tbl_warehouse_issue_return.item_id')
                ->join('tbl_product_color', 'tbl_product_color.id', '=', 'tbl_item.product_color_id')
        ...
        $query = $query->select('tbl_invoice.invoice_no as invoice_id', 'tbl_retailer_order.order_no', 'tbl_imei.imei1'
                        , 'tbl_imei.imei2', 'tbl_product_color.color', 'tbl_product.name as p_name', 'tbl_retailer.retailer_no'
                        , 'tbl_retailer_outlet.name as outlet_name', 'tbl_retailer_outlet.phone_no', 'tbl_retailer_outlet.address'
                        , 'tbl_warehouse_issue_return.created_at as return_time', 'tbl_city.name as city_name'
                        , 'tbl_state.name as region', 'tbl_region.name as zone'
                )
                ->orderBy('tbl_warehouse_issue_return.created_at', 'desc')->orderBy('tbl_product.id', 'asc')
                ->orderBy('tbl_product_color.id', 'asc')
                ->get();
	  ...
	  public function get_sale_return_imeis_details(Request $request){
        if ( $request->input('client') ) {
            return DB::table('tbl_warehouse_issue_return')
                ->join('tbl_imei', 'tbl_warehouse_issue_return.item_id', '=', 'tbl_imei.item_id')
                ->join('tbl_item', 'tbl_item.id', '=', 'tbl_warehouse_issue_return.item_id')
                ->join('tbl_product_color', 'tbl_product_color.id', '=', 'tbl_item.product_color_id')
                ->join('tbl_product', 'tbl_product.id', '=', 'tbl_product_color.product_id')
        ...
        if ($searchValue) {
            $query->where(function($query) use ($searchValue) {
                ...
                orWhereHas('tbl_imei', function ( $q ) use ($searchValue){
    ...
    public function create_month_zero() {
        $month = [];
        for ($i = 1; $i <= 31; $i++) {
            $month[$i] = 0;
        }
