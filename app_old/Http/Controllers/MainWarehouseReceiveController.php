...
class MainWarehouseReceiveController extends Controller
{
  public function __construct()
  {
   ...
   public function product_uploading_with_imies_view(){
    ...
    //FOr new consignment upload
    public function uploads_items_in_excel(Request $request){
        $userId=Auth::user()->id;
    ...
    public function verify_imeis(Request $request){
        $sheet_one=array();
        $sheet_two=array();
        if(Input::hasFile('import_file_one')){
            $path = Input::file('import_file_one')->getRealPath();
            ...
            if(!empty($data1) && $data1->count()){
                ...
                if($is_uploaded==0){
                    foreach ($data1 as $key => $value) {
                        $sheet_one[]=['imei1'=>$value->imei1, 'imei2'=> $value->imei2];
                    }
                }
                else{
                    Session::flash('import_error_message', "Duplicate Imeis Found");
        ...
        if(Input::hasFile('import_file_two')){
            $path = Input::file('import_file_two')->getRealPath();
            $data1 = Excel::load($path, function($reader) {
            })->get();
            ...
            if(!empty($data1) && $data1->count()){
                ...
                if($is_uploaded==0){
                    foreach ($data1 as $key => $value) {
                        $sheet_two[]=['imei1'=>$value->imei1, 'imei2'=> $value->imei2];
                    }
      ...
      /* for($i=0; $i<count($sheet_one)-1; $i++){
            for($j=0; $j<count($sheet_two)-1; $j++){
                if($sheet_one[$i]['imei1']==$sheet_two[$j]['imei1'] || $sheet_one[$i]['imei1']==$sheet_two[$j]['imei2']){
                    array_push($founded_imeis,  (string)$sheet_one[$i]['imei1']);
                    break;
                }
            }
           // echo $sheet_one[$i]['imei1'];
       }

        for($i=0; $i<count($sheet_one)-1; $i++){
            for($j=0; $j<count($sheet_two)-1; $j++){
                if($sheet_one[$i]['imei2']==$sheet_two[$j]['imei1'] || $sheet_one[$i]['imei2']==$sheet_two[$j]['imei2']){
                    array_push($founded_imeis,  (string)$sheet_one[$i]['imei2']);
                    break;
                }
            }
            // echo $sheet_one[$i]['imei1'];
        }*/
      /*  echo count($founded_imeis)."<br/>";
       print_r($founded_imeis);*/

     // $result= print_r($sheet_two);

       // $result = $this->check_diff_multi($sheet_two, $sheet_one);

           $result= $this->mis_match_imeis($sheet_one, $sheet_two);
        /*print '<pre>';
          print_r($result);
        print '</pre>';*/
    ...
    public function mis_match_imeis($sheet_one, $sheet_two){
      ini_set('memory_limit', '-1');
    $not_match_imeis=[];
    $number_count=0;
    // echo 'Sheet one size: '.count($sheet_one);
    // echo 'Sheet two size: '.count($sheet_two);
      for($i=0; $i<count($sheet_two); $i++){
        // bool $found = false;
        // global $found = false;
          for($j=0; $j<count($sheet_one); $j++){
              if($sheet_two[$i]['imei1']==$sheet_one[$j]['imei1'] || 
                ($sheet_two[$i]['imei1']==$sheet_one[$j]['imei2'] && $sheet_two[$i]['imei2']==$sheet_one[$j]['imei1'])){
                ++$number_count;
                break;
                // echo 'Sheet Two IMEI: '.$sheet_two[$i]['imei1'].'<br/>';
                // echo 'Sheet One IMEI: '.$sheet_one[$j]['imei1'].'<br/><br/>';
              } else {
                    if($j == count($sheet_one) - 1) {
                      echo 'IMEI '.$sheet_two[$i]['imei1']. ' not found <br/>';
                  }
              }
          }
      }
