...
class RetailerOutletController extends Controller {
    ...
    public function get_all_outlets_dataTable_old(Request $request) {

        if ($request->input('client')) {
            return RetailerOutlet::with('retailer', 'city', 'region')->where('is_deleted', 0)->get();
        }
        ...
        $query = RetailerOutlet::with('retailer', 'city', 'region')->orderBy($columns[$column], $dir);
        if ($searchValue) {
