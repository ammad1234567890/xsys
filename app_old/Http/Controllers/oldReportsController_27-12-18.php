...
class ReportsController extends Controller {
    ...
    public function get_outstanding_ledger(Request $request) {
        ...
        $query = DB::table('tbl_retailer_outlet')
                ->select(
                        DB::raw("(SELECT ROUND(SUM(Credit)-sum(COALESCE(Collection,0))) FROM `tbl_ledger` WHERE retailer_id=tbl_retailer_outlet.id) as outstanding")
...
//        }
    ...
    public function get_collection(Request $request) {
        ...
        $query = DB::table('tbl_retailer_outlet')
                ->select('tbl_retailer_outlet.id as outlet_id', 'tbl_retailer_outlet.city_id as outlet_city_id'
                        ...
                        , 'tbl_retailer_collection.created_at as collection_created_at'
    ...
    public function sale_return_details(Request $request) {
	    ...
	    public function sale_return_imeis_details2(Request $request) {
        $query = DB::table('tbl_warehouse_issue_return')
                ->join('tbl_imei', 'tbl_warehouse_issue_return.item_id', '=', 'tbl_imei.item_id')
                ->join('tbl_item', 'tbl_item.id', '=', 'tbl_warehouse_issue_return.item_id')
                ->join('tbl_product_color', 'tbl_product_color.id', '=', 'tbl_item.product_color_id')
                ...
                ->join('tbl_region', 'tbl_city.id', '=', 'tbl_region.city_id');
//->where('tbl_retailer_outlet.id', '=', $request->outlet_id);
        if ($request->warehouse_id != '') {
            $query = $query->where('tbl_retailer_order.warehouse_id', '=', $request->warehouse_id);
        }
        ...
        $query = $query->select('tbl_invoice.invoice_no as invoice_id', 'tbl_retailer_order.order_no', 'tbl_imei.imei1'
                        ...
                        , 'tbl_retailer_outlet.name as outlet_name', 'tbl_retailer_outlet.phone_no', 'tbl_retailer_outlet.address'
                        , 'tbl_warehouse_issue_return.created_at as return_time', 'tbl_city.name as city_name', 'tbl_state.name as region', 'tbl_region.name as zone')
                ->orderBy('tbl_warehouse_issue_return.created_at', 'desc')->orderBy('tbl_product.id', 'asc')
                ->orderBy('tbl_product_color.id', 'asc')
                ->get();
        if (sizeof($query)) {
